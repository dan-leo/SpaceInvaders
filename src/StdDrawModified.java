/*************************************************************************
 *  Compilation:  javac StdDrawModified.java
 *  Execution:    java StdDrawModified
 *
 ...
 *  @author Kevin Wayne
 */
public final class StdDrawModified implements ActionListener, MouseListener, MouseMotionListener, KeyListener, MouseWheelListener, MouseInputListener {
    ...
    private static StdDrawModified std = new StdDrawModified();
    ...
    *************************************************************************/
    ...
    public static Image getImage(String filename) {
        ...
        if ((icon == null) || (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
            URL url = StdDrawModified.class.getResource(filename);
    ...
    public static void picture(double x, double y, String s, double w, double h) {
    	if (debug_time) System.out.printf("{{");
		long[] now = new long[10];
		if (debug_time) now[0] = System.currentTimeMillis();
        Image image = getImage(s);
        if (debug_time) now[1] = System.currentTimeMillis();
        double xs = scaleX(x);
        double ys = scaleY(y);
        ...
        if (ws <= 1 && hs <= 1) pixel(x, y);
        	...
        	if (debug_time) now[2] = System.currentTimeMillis();
            offscreen.drawImage(image, (int) Math.round(xs - ws/2.0),
                                       (int) Math.round(ys - hs/2.0),
                                       (int) Math.round(ws),
                                       (int) Math.round(hs), null);
            if (debug_time) now[3] = System.currentTimeMillis();
        }
        draw();
        if (debug_time) now[4] = System.currentTimeMillis();
		for (int i = 1; i < 4; i++)
		{
			if (debug_time) System.out.printf("%-3d/ ", now[i] - now[i - 1]);
		}
    }
    
    public static void picture(double x, double y, Image image, double w, double h) {
    	long[] now = new long[10];
		if (debug_time) now[0] = System.currentTimeMillis();
        // Image image = getImage(s);
		if (debug_time) now[1] = System.currentTimeMillis();
        ...
        double ws = factorX(w);
        double hs = factorY(h);
        if (ws < 0 || hs < 0) throw new IllegalArgumentException("image " + image.toString() + " is corrupt");
        if (ws <= 1 && hs <= 1) pixel(x, y);
        else {
        	if (debug_time) now[2] = System.currentTimeMillis();
            offscreen.drawImage(image, (int) Math.round(xs - ws/2.0),
     ...
     * @param x the center x-coordinate of the image
     * @param y the center y-coordinate of the image
     * @param image the image/picture.
     * @param w the width of the image
     * @param h the height of the image
     * @param degrees is the number of degrees to rotate counterclockwise
     * @throws IllegalArgumentException if the image is corrupt
     */
    public static void picture(double x, double y, Image image, double w, double h, double degrees) {
        double xs = scaleX(x);
        double ys = scaleY(y);
        double ws = factorX(w);
        double hs = factorY(h);
    ...
    *************************************************************************/
    ...
    public void actionPerformed(ActionEvent e) {
        FileDialog chooser = new FileDialog(StdDrawModified.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
        chooser.setVisible(true);
        String filename = chooser.getFile();
        if (filename != null) {
            StdDrawModified.save(chooser.getDirectory() + File.separator + chooser.getFile());
        }
    }
    ...
    *************************************************************************/
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (mouseLock) {
            mouseX = StdDrawModified.userX(e.getX());
            mouseY = StdDrawModified.userY(e.getY());
            mousePressed = true;
        }
    }
    ...
    *************************************************************************/
    ...
    public static void main(String[] args) {
        StdDrawModified.square(.2, .8, .1);
        StdDrawModified.filledSquare(.8, .8, .2);
        StdDrawModified.circle(.8, .2, .2);

        StdDrawModified.setPenColor(StdDrawModified.BOOK_RED);
        StdDrawModified.setPenRadius(.02);
        StdDrawModified.arc(.8, .2, .1, 200, 45);

        // draw a blue diamond
        StdDrawModified.setPenRadius();
        StdDrawModified.setPenColor(StdDrawModified.BOOK_BLUE);
        double[] x = { .1, .2, .3, .2 };
        double[] y = { .2, .3, .2, .1 };
        StdDrawModified.filledPolygon(x, y);

        // text
        StdDrawModified.setPenColor(StdDrawModified.BLACK);
        StdDrawModified.text(0.2, 0.5, "black text");
        StdDrawModified.setPenColor(StdDrawModified.WHITE);
        StdDrawModified.text(0.8, 0.8, "white text");
        StdDrawModified.mouseWheel();
    }
